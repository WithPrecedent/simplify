...
class ReMatch(object):
    ...
    pandas dataframes and series with regular expressions or ordinary strings
    as keys.
    ...
    Because the methods iterrate through the expressions dataframes, any
    efficiency gains of vectorization in the target dataframes or series are
    lost with very long lookup dataframes. Further, because regular expressions
    ...
    python lists, or imported from a .csv file into a pandas dataframe.

    Using the match method, the user can either:
        1) Pass a string (within a pandas series or freestanding) to find a
        match and store the result in a pandas series;
            or;
    ...
    Regular expressions are ordinarily used only as keys, but not values,
    unless the reverse_dict option is set to True. Regular expressions may be
    ...
    if out_type is 'pattern', a single column is used or created with the
    header name passed in 'out_col.' The return is the matched values from the
